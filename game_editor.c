...
void mouse_click(int in_game_x, int in_game_y_inverted, struct EditorState* r_editor_state, struct GameState* r_game_state) {
    ...
    if (r_editor_state->click_state == ADD_BLOCK) {
        struct Block* mouse_block = get_world_block_for_location(in_game_x, in_game_y_inverted, r_game_state);
        if (mouse_block != NULL) {
        ...
        // struct Block* block = get_world_block_for_location(left, top, r_game_state);
        // printf("block to check is %d,%d effect flags %d\n", block->block_x, block->block_y, block->effects_flags);
        // block = get_world_block_for_location(right, top, r_game_state);
        // printf("block to check is %d,%d effect flags %d\n", block->block_x, block->block_y, block->effects_flags);
        // block = get_world_block_for_location(left, bottom, r_game_state);
        // printf("block to check is %d,%d effect flags %d\n", block->block_x, block->block_y, block->effects_flags);
        // block = get_world_block_for_location(right, bottom, r_game_state);
        // printf("block to check is %d,%d effect flags %d\n", block->block_x, block->block_y, block->effects_flags);

        uint32_t entity_fits_in_blocks = !(
                (get_world_block_for_location(left, top, r_game_state)->effects_flags & EFFECT_FLAG_SOLID)
            |   (get_world_block_for_location(right, top, r_game_state)->effects_flags & EFFECT_FLAG_SOLID)
            |   (get_world_block_for_location(left, bottom, r_game_state)->effects_flags & EFFECT_FLAG_SOLID)
            |   (get_world_block_for_location(right, bottom, r_game_state)->effects_flags & EFFECT_FLAG_SOLID)
        );
        ...
        new_entity->x_velocity_pixels_per_second = 3;
